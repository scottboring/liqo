...
func (r *VirtualNodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, virtualNode); err != nil {
		...
		pterm.FgYellow.Printfln("There is a virtual-node called '%s' in '%s'", req.Name, req.Namespace)
	}

	r.addVirtualNodeFinalizer(ctx, virtualNode)
...
func (r *VirtualNodeReconciler) addVirtualNodeFinalizer(ctx context.Context, virtualNode *virtualkubeletv1alpha.VirtualNode) {
